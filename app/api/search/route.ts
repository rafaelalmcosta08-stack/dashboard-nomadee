...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const profilesPromise = (async () => {
      try {
        const { data } = await admin.from('profiles').select('id, username, qra, patente, role')
        return data || []
      } catch {
        return []
      }
    ...
    const coursesPromise = (async () => {
      try {
        const { data } = await admin.from('cursos').select('*')
    ...
    const editaisPromise = (async () => {
      try {
        const { data } = await admin.from('editais').select('*')
        if (data && data.length > 0) return data
    ...
    const avisosPromise = (async () => {
      try {
        const { data } = await admin.from('avisos').select('*')
        if (data && data.length > 0) return data
