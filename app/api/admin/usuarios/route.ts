...
export async function PATCH(req: NextRequest) {
  ...
  try {
    if (status !== undefined) await admin.from('profiles').update({ status }).eq('id', id)
    if (role !== undefined) await admin.from('profiles').update({ role }).eq('id', id)
    if (patente !== undefined) await admin.from('profiles').update({ patente }).eq('id', id)
  } catch (_) {}
