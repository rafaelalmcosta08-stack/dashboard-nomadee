...
function getMinPatenteInfo(rawPatentes: any) {
  ...
  // Permissões de visualização do item específico para o usuário logado (Patente Mínima + Unidade)
  function canUserUseItem(item: Fardamento) {
    ...
    // Verifica patente mínima
    const myPatente = profile?.patente || 'Recruta'
    const info = getMinPatenteInfo(item.allowedPatentes)

    if (info.isAllByUnit) return unitAllowed

    const userIdx = PATENTES.indexOf(myPatente)
    // O usuário pode usar o item se sua patente for maior ou igual à patente mínima
  ...
  const sortedItems = [...filteredItems].sort((a, b) => {
    const infoA = getMinPatenteInfo(a.allowedPatentes)
    const infoB = getMinPatenteInfo(b.allowedPatentes)
                  ...
                  <div>
                    ...
                    <div className="space-y-2 mt-3.5 border-t border-border/10 pt-3">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {(() => {
                            const minInfo = getMinPatenteInfo(item.allowedPatentes)
