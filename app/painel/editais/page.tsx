...
export interface Edital {
  ...
  description: string
  requirements?: string
  unidade: 'GRAER' | 'ROCAM' | 'GRR' | 'BOPE' | 'CHOQUE' | 'DPJM' | 'RPM' | 'APM' | 'Geral'
...
export default function EditaisPage() {
  ...
  const isAllPowerfulEditalPublisher =
    ...
    myCargos.includes('Diretor DPJM') ||
    myCargos.includes('Diretor Corregedoria') ||
    myCargos.includes('Diretor APM')

  const isBopeCommand = myCargos.includes('Comando Bope') || myCargos.includes('Comando BOPE')
  const isChoqueCommand = myCargos.includes('Comando Choque') || myCargos.includes('Comando CHOQUE') || myCargos.includes('Comando Core')
  const isGrrCommand = myCargos.includes('Comando GRR') || myCargos.includes('Comando GAR')
  const isGraerCommand = myCargos.includes('Comando GRAER') || myCargos.includes('Comando GAEP')
  const isRocamCommand = myCargos.includes('Comando ROCAM') || myCargos.includes('Comando GTM')
  ...
  const getAvailableUnitsForUser = () => {
    if (isAllPowerfulEditalPublisher) {
      return ['Geral', 'GRAER', 'ROCAM', 'GRR', 'BOPE', 'CHOQUE', 'DPJM', 'APM']
    }
    const units: string[] = []
    if (isBopeCommand) units.push('BOPE')
    if (isChoqueCommand) units.push('CHOQUE')
    if (isGrrCommand) units.push('GRR')
    if (isGraerCommand) units.push('GRAER')
    if (isRocamCommand) units.push('ROCAM')
    return units
  }
  ...
  const canManageSpecificEdital = (edital: Edital) => {
    ...
    if (isChoqueCommand && (edital.unidade === 'CHOQUE' as any || edital.unidade === 'CORE' as any)) return true
    if (isGrrCommand && (edital.unidade === 'GRR' as any || edital.unidade === 'GAR' as any)) return true
    if (isGraerCommand && (edital.unidade === 'GRAER' as any || edital.unidade === 'GAEP' as any)) return true
    if (isRocamCommand && (edital.unidade === 'ROCAM' as any || edital.unidade === 'GTM' as any)) return true
            ...
            <div className="space-y-12">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card/40 border border-border/40 p-4 rounded-xl backdrop-blur-sm">
                <div>
                  ...
                  <span className="text-xs text-muted-foreground mt-0.5 block">Exiba todos os editais ou busque pelo título do edital desejado.</span>
                </div>
                <div className="flex items-center gap-2 flex-wrap self-start sm:self-center">
                  ...
                  <div className="relative flex items-center min-w-[200px] sm:min-w-[240px]">
                    ...
                    <input
                      ...
                      placeholder="Buscar título do edital..."
                      className="h-8.5 w-full pl-8 pr-3 text-xs bg-background/60 border border-border/60 rounded-lg outline-none focus:border-foreground transition-colors"
                    />
                  </div>
                </div>
              </div>
