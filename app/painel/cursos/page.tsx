...
export default function CursosPage() {
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      const instructorQra = instructorId.trim()
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      const editInstructorQra = editInstructorId.trim()
  ...
  const filteredCourses = courses.filter(c => {
    ...
    return (
      ...
      normalizeStr(c.description).includes(query) ||
      normalizeStr(c.requirements || '').includes(query) ||
      normalizeStr(c.instructorQra || '').includes(query) ||
      normalizeStr(c.creatorQra).includes(query)
    )
  })
  ...
  const getOfficerEditalHistory = (officerId: string) => {
    return editais
      .filter(e => e.subscribers.some((sub: any) => sub.userId === officerId))
      .map(e => {
              ...
              <form onSubmit={handleSaveEdit} className="mt-6 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="mt-1.5 w-full rounded-lg border border-border/80 bg-background/50 px-3.5 py-2.5 text-sm outline-none focus:border-foreground"
                    placeholder="Ex: Instrutor Silva"
                  />
                </div>
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card/40 border border-border/40 p-4 rounded-xl backdrop-blur-sm">
                <div>
                  ...
                  <span className="text-xs text-muted-foreground mt-0.5 block">Exiba todos os cursos ou busque pelo título do curso desejado.</span>
                </div>
                <div className="flex items-center gap-2 flex-wrap self-start sm:self-center">
                  ...
                  <div className="relative flex items-center min-w-[200px] sm:min-w-[240px]">
                    ...
                    <input
                      ...
                      placeholder="Buscar título do curso..."
              ...
              {searchTitle.trim() !== '' && (
                <div className="flex items-center justify-between rounded-xl bg-primary/10 border border-primary/25 p-3.5 text-xs">
                  <span className="font-semibold text-foreground flex items-center gap-2">
                    ...
                    Exibindo resultados para "<strong>{searchTitle}</strong>" ({filteredCourses.length} curso{filteredCourses.length !== 1 ? 's' : ''} encontrado{filteredCourses.length !== 1 ? 's' : ''})
                  </span>
                  <button
                    onClick={() => setSearchTitle('')}
              ...
              {searchTitle.trim() !== '' && filteredCourses.length === 0 ? (
                <div className="rounded-xl border border-dashed border-border/60 p-8 text-center bg-card/10">
                  <Search className="mx-auto h-8 w-8 text-muted-foreground/50" />
                  <p className="mt-3 text-sm font-semibold text-foreground">Nenhum curso encontrado</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    Não encontramos nenhum curso com o termo "{searchTitle}". Tente buscar por outro título, palavra-chave ou instrutor.
                  </p>
                  <button
                    onClick={() => setSearchTitle('')}
